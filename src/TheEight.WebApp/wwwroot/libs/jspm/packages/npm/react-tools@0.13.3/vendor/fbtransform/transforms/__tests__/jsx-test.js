...
describe('jsx', function() {
  ...
  function runWithAttribute(inlineAttribute, isLast) {
    ...
    jsx.renderJSXExpressionContainer(function() {}, attribute.value, isLast, [], state);
    return state;
  }
  ...
  it('should render simple literal with single space before' + ' from multiple spaces', function() {
    var state = runWithLiteral(' \t  a', true);
    expect(state.g.buffer).toEqual('"    a"');
  });
  ...
  it('should render simple literal with single space after' + ' from multiple spaces', function() {
    var state = runWithLiteral('a  \t \t ', true);
    expect(state.g.buffer).toEqual('"a      "');
  });
  it('should render multiline literal as last', function() {
    var state = runWithLiteral('  sdfsdfsdf\n' + '  sdlkfjsdfljs\n' + '   ', true);
    expect(state.g.buffer).toEqual('"  sdfsdfsdf" + \' \' +\n' + '  "sdlkfjsdfljs"\n' + '   ');
  });
  ...
  it('should render attribute expressions with commas before trailing ' + 'whitespace', function() {
  ...
  it('should not render commas after empty child expressions even if they\'re' + 'not last', function() {
    var source = '<div>{/*comment*/}</div>';
    var ast = esprima.parse(source, {range: true});
    var child = ast.body[0].expression.children[0];
    var state = utils.createState(source, child);
